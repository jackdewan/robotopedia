...
export const richTextRenderOptions = {
  renderNode: {
    ...
    [BLOCKS.PARAGRAPH]: (node, children) => {
      return (
        <p className="text-gray-700 text-base dark:text-gray-400 leading-relaxed mb-4 text-justify">
          {children}
        </p>
      );
    },
...
export default function PostSlug() {
  const { title, description, tag, blogBody, sys, canonicalUrl } =
    useLoaderData();
  let canonicalName = '';
  if (canonicalUrl) {
        ...
        <div className="post px-4 sm:px-0">
          {canonicalUrl ? (
            <p className="text-lg mb-6 italic">
              ...
              <a href={canonicalUrl}>{canonicalName}</a>.
            </p>
          ) : null}
